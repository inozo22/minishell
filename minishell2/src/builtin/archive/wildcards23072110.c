...
/*   Updated: 2023/07/21 10:00:16 by nimai            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	valid_wildcard(char *input, char *dirname)
{
	...
	while (dirname[i[1]] && input[i[0]] && i[1] < (int)ft_strlen(dirname))
	{
		...
		printf("Line: %d\n", __LINE__);
//		check_input_tab(input, tab, dirname, i);
		if (ft_strlen(*tab) == 1)
		{
			printf("Line: %d\n", __LINE__);
			if (*tab[0] == dirname[i[1]])
			{
				i[3]++;
				i[0]++;
				i[1]++;
		...
		else if (ft_strncmp(*tab, &dirname[i[1]], (int)ft_strlen(*tab)) == 0)
		{
			printf("Line: %d\n", __LINE__);
			i[3] += ft_strlen(*tab);
			printf("Line: %d\n", __LINE__);
			...
			tab++;
			printf("Line: %d\n", __LINE__);
			if (!*tab && ((i[1] >= (int)ft_strlen(dirname)) || (i[3] == i[2] && input[i[0] + 1] == '*')))
			{
				printf("Line: %d\n", __LINE__);
				//myfree_array(tab);
				printf("Line: %d\n", __LINE__);

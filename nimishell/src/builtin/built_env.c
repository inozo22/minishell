...
/*   Updated: 2023/06/26 10:44:21 by nimai            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	built_env(char **input, t_data *data)
{
	...
	if (av_amount(input) > 1)
		return (error_input(input[0], "with no options or arguments are required"), 1);
 ...
 * 230601nimai: there is no "OLDPWD"? -> When there is record of moving, it will be appeared->230614nimai: done
 * 
 * MEMORY LEAKS

...
int	obtain_expanded_len(char *preceding, char *following, char *expanded)
{
	int	len;
...
//From here
	...
	{
//		len =  obtain_expanded_len(preceding, following, expanded);
		if (*preceding && *following && expanded)
			len = ft_strlen(preceding) + ft_strlen(expanded) - 2;
		else if (*preceding && expanded)
			len = ft_strlen(preceding) + ft_strlen(expanded) - 1;
		...
		len = ft_strlen(preceding) - 1;
//until this line move to obtain_expanded_len
